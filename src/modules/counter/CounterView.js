...
import { getRovers, getPosts } from '../../utils/api';
...
class CounterView extends Component {
  ...
  componentDidMount() {
    this.formatData(demoData);
  }
  /*componentWillMount() {
    getRovers().then((res) => {
      this.setState({
        rovers: res.rovers,
        roversName: res.rovers[0].name
      })
    });

    getPosts().then((res) => {
      this.setState({
        posts: this.state.posts.cloneWithRows(res.data.posts)
      });
    });

    this.formatData(demoData).then((payload) => {
     console.log(payload);  
     const { dataBlob, sectionIds, rowIds } = payload;
     this.setState({
   ...
   formatData(demoData) {
     ...
     const api = 'https://randomuser.me/api/?page=3&results=15&seed=a';
      ...
      const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
       ...
       for (let sectionId = 0; sectionId < alphabet.length; sectionId++) {
         ...
         const users = demoData.filter((user) => user.name.first.toUpperCase().indexOf(currentChar) === 0);
...
/*<Text>{this.state.roversName}</Text>
        <TouchableOpacity
